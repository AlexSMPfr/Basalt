...
public class RegistryTagRewriter<T> extends SearchReplaceRewriter {
    ...
    protected void insert(SearchMetadata metadata, StringBuilder builder) {
        Iterator<? extends TagKey<T>> referenceIterator = this.registry.getTagNames().sorted(Formatting.alphabeticKeyOrder(reference -> reference.location().getPath())).iterator();

        while (referenceIterator.hasNext()) {
            TagKey<T> tagKey = referenceIterator.next();
    ...
    protected String rewriteFieldName(TagKey<T> reference) {
        return Formatting.formatKeyAsField(reference.location().getPath());
    }

    protected String rewriteFieldValue(TagKey<T> reference) {
        return "%s(%s)".formatted(this.fetchMethod, quoted(reference.location().getPath()));
    }
}
